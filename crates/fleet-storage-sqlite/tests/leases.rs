...
#[tokio::test]
async fn failed_provision_attempt_marks_lease_terminal_for_replacement() {
    let (_dir, store, leases) = setup().await;
    let lease = leases
        .create(
            &NewLease {
    ...
    let provision = fleet_storage_sqlite::LabRepository::new(store.pool().clone())
        .create(
            &NewProvision {
                template_version_id: lease.template_version_id.clone(),
                lease_id: Some(lease.id.clone()),
                idempotency_key: Some("operator:lease-failure".to_owned()),
            },
            NOW,
        )
        .await
    ...
    assert!(
        leases
            .fail_provisioning(&lease.id, &provision.id)
            .await
            .unwrap()
    );
    let failed = leases.get(&lease.id).await.unwrap();
