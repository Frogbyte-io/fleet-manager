...
pub enum Command {
    ...
    MachinesList {
        ...
        /// Continue after this machine id.
        cursor: Option<String>,
        /// Maximum entries to request.
        limit: Option<u32>,
    },
    ...
    /// Read one machine's last observed skills inventory.
    ...
    /// Publish a recipe draft: freeze an immutable version.
    ImagesPublish {
        /// The recipe's identity.
        recipe_id: String,
    },
    /// List a recipe's published versions.
    ImagesVersions {
        /// The recipe's identity.
        recipe_id: String,
    },
    ...
    /// Start provisioning a requested lease with its configured Proxmox account.
    LabLeaseProvision {
        /// The lease to provision.
        lease_id: String,
        /// The configured Proxmox account to use.
        account_id: String,
    },
    ...
    /// Extend a ready Lab lease by seconds, bounded by its maximum lifetime.
    LabLeaseExtend {
        /// The lease's identity.
        lease_id: String,
    ...
    /// Show one version with its structured view.
    ImagesVersion {
        /// The version's identity.
        version_id: String,
    },
    /// Start a build of one published version as a durable operation.
    ImagesBuild {
...
fn parse_lab_command(verb: &str, rest: &[&str]) -> Result<Command, CliError> {
    match verb {
        ...
        "provision-lease" => match rest {
            [lease_id, "--account", account_id] => Ok(Command::LabLeaseProvision {
        ...
        "extend" => match rest {
            [lease_id, "--seconds", seconds] => {
                let by_seconds = seconds.parse::<u32>().map_err(|_| CliError {
                    message: "--seconds must be a positive integer".to_owned(),
                })?;
                if by_seconds == 0 {
                    return Err(CliError {
                        message: "--seconds must be a positive integer".to_owned(),
    ...
    match verb {
        ...
        "create" => match rest {
            [
                ...
                "--source",
                source,
            ] => Ok(Command::ImagesCreate {
                name: (*name).to_owned(),
                description: (*description).to_owned(),
                node: (*node).to_owned(),
                storage_pool: (*pool).to_owned(),
                source: (*source).to_owned(),
            }),
            _ => Err(CliError { message: usage() }),
        },
        "publish" => match rest {
            [recipe_id] => Ok(Command::ImagesPublish {
                recipe_id: (*recipe_id).to_owned(),
            }),
            _ => Err(CliError { message: usage() }),
        },
        "versions" => match rest {
            [recipe_id] => Ok(Command::ImagesVersions {
                recipe_id: (*recipe_id).to_owned(),
            }),
            _ => Err(CliError { message: usage() }),
        },
        ...
        "version" => match rest {
            [version_id] => Ok(Command::ImagesVersion {
                version_id: (*version_id).to_owned(),
            }),
            _ => Err(CliError { message: usage() }),
        },
...
mod event_output_tests {
    ...
    #[test]
    fn skills_read_commands_target_the_read_model() {
        let list = parse(&["skills".into(), "list".into(), "box".into()]).unwrap();
        assert!(matches!(list.command, Command::SkillsList { ref machine } if machine == "box"));
        assert_eq!(
            request_for(&list.command).unwrap().1,
            "/api/v1/machines/box/skills"
        );
        let matrix = parse(&["skills".into(), "matrix".into()]).unwrap();
        assert!(matches!(matrix.command, Command::SkillsMatrix));
        assert_eq!(
...
fn render_skills(payload: &Value) -> String {
    ...
    for row in rows {
        let machine = row["machineId"].as_str().unwrap_or("?");
        let status = row["availability"].as_str().unwrap_or("unknown");
        let stale = row["stale"].as_bool().unwrap_or(false);
        let _ = writeln!(
            output,
            "{machine}: {status}{}",
            if stale { " (stale)" } else { "" }
        );
        for skill in row["data"]["skills"].as_array().into_iter().flatten() {
            let agents = skill["deployedTo"]
                .as_array()
                .into_iter()
                .flatten()
...
fn request_for(command: &Command) -> Result<RequestShape, CliError> {
    Ok(match command {
        ...
        Command::SkillsList { machine } => (
            reqwest::Method::GET,
            format!("/api/v1/machines/{machine}/skills"),
            Vec::new(),
            None,
        ),
        Command::SkillsMatrix => (
            reqwest::Method::GET,
        ...
        Command::ImagesCreate {
            ...
            Some(serde_json::json!({
                ...
                "source": source,
                "content": read_stdin_to_eof("the recipe content as JSON")?,
            })),
        ),
        Command::ImagesPublish { recipe_id } => (
            reqwest::Method::POST,
            format!("/api/v1/images/recipes/{recipe_id}/publish"),
            Vec::new(),
            None,
        ),
        Command::ImagesVersions { recipe_id } => (
            reqwest::Method::GET,
            format!("/api/v1/images/recipes/{recipe_id}/versions"),
            Vec::new(),
            None,
        ),
        ...
        Command::LabLeaseExtend {
            ...
            reqwest::Method::POST,
            format!("/api/v1/lab/leases/{lease_id}/extend"),
            Vec::new(),
            Some(serde_json::json!({ "bySeconds": by_seconds })),
        ),
        Command::LabSweep => (
            reqwest::Method::POST,
        ...
        Command::ImagesVersion { version_id } => (
            reqwest::Method::GET,
            format!("/api/v1/images/versions/{version_id}"),
            Vec::new(),
            None,
        ),
...
fn render_images(value: Option<&Value>) -> String {
    ...
    // Recipes/versions lists: one row per entry.
    if let Some(items) = value.get("items").and_then(Value::as_array) {
        let mut lines = vec![format!(
            "{:<38} {:<24} {:<20} {}",
            "ID", "NAME", "SOURCE", "PUBLISHED FROM"
        )];
        for item in items {
            lines.push(format!(
                "{:<38} {:<24} {:<20} {}",
                ...
                item["publishedFrom"].as_str().unwrap_or("-"),
            ));
        }
        if items.is_empty() {
            lines.push("(no image recipes)".to_owned());
        }
        return lines.join("\n");
    }
    // Single recipe/version/answers: key facts only.
    let mut lines = Vec::new();
    for (key, val) in value.as_object().into_iter().flatten() {
        let rendered = match val {
            Value::String(text) => text.clone(),
