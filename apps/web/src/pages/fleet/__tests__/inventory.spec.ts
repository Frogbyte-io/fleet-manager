...
describe('specLine', () => {
  it('joins the known facts', () => {
    const built = buildInventory({ machines: [machine()], machinesError: null, proxmox: [], tailnet: { configured: false, devices: null, error: null } })
  ...
  it('skips unknown parts', () => {
    const built = buildInventory({
      machinesError: null,
      tailnet: { configured: false, devices: null, error: null },
...
describe('buildInventory', () => {
  it('counts host guests and templates, excluding templates from guests', () => {
    const built = buildInventory({
      ...
      machinesError: null,
      tailnet: { configured: false, devices: null, error: null },
      proxmox: [{ accountId: 'acc1', accountName: 'homelab', confirmed: true, loading: false, discovery: discovery(), guests: null, error: null }],
    })
  ...
  it('enriches guests by accountId+vmid', () => {
    const built = buildInventory({
      ...
      machinesError: null,
      tailnet: { configured: false, devices: null, error: null },
      proxmox: [{ accountId: 'acc1', accountName: 'homelab', confirmed: true, loading: false, discovery: discovery(), guests: [guest()], error: null }],
    })
  ...
  it('does not enrich when the vmid belongs to another account', () => {
    const built = buildInventory({
      machines: [],
      machinesError: null,
      tailnet: { configured: false, devices: null, error: null },
      proxmox: [{ accountId: 'acc1', accountName: 'homelab', confirmed: true, loading: false, discovery: discovery(), guests: [guest({ vmid: 999 })], error: null }],
    })
  ...
  it('lists uncorrelated devices in tailnetOnly', () => {
    const built = buildInventory({
      ...
      machinesError: null,
      proxmox: [],
      tailnet: { configured: true, devices: [device()], error: null },
    })
  ...
  it('marks unconfirmed accounts untrusted with no discovery data', () => {
    const built = buildInventory({
      ...
      machinesError: null,
      tailnet: { configured: false, devices: null, error: null },
      proxmox: [{ accountId: 'acc1', accountName: 'homelab', confirmed: false, loading: false, discovery: null, guests: null, error: null }],
    })
  ...
  it('propagates a discovery error as a source error', () => {
    const built = buildInventory({
      ...
      machinesError: null,
      tailnet: { configured: false, devices: null, error: null },
      proxmox: [{ accountId: 'acc1', accountName: 'homelab', confirmed: true, loading: false, discovery: null, guests: null, error: 'unreachable' }],
    })
  ...
  it('marks a confirmed account with no discovery and no error as loading, not untrusted', () => {
    const built = buildInventory({
      machines: [],
      machinesError: null,
      proxmox: [{ accountId: 'acc1', accountName: 'homelab', confirmed: true, loading: true, discovery: null, guests: null, error: null }],
      tailnet: { configured: false, devices: null, error: null },
    })
    const source = built.sources.find(s => s.key === 'proxmox:acc1')
