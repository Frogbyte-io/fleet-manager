...
FM-603 (epic #12): Proxmox template, clone, and snapshot operations — only after idempotency and destructive-operation review; snapshot/revert/delete behind an explicit destructive-operation permission; compensation and reconciliation when a clone or snapshot fails midway; recorded/simulated API tests plus the real-cluster suite. FM-602's lifecycle executor and UPID polling are the foundation; remote task cancellation (left running in FM-602) belongs to this epic's review. Create the issue, post the approach, branch `fm-603-*`. The M7 Lab epics wait on M6.
