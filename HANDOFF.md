...
FM-602 (epic #11): Proxmox guest lifecycle and task operations — start/stop/shutdown/reboot as durable audited operations over the FM-108/FM-109 kernel, UPID task polling with progress/cancellation/deadlines (Fleet-owned loops, no wall-clock flake), partial-node and privilege failures surfaced per resource. FM-600's accounts/trust flow and FM-601's guest discovery are the foundation. Create the issue, post the approach, branch `fm-602-*`. Destructive operations (template/clone/snapshot, epic #12) wait for this epic per the dependency order.
