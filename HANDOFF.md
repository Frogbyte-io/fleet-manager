...
M6 is code-complete (epics #9–#12 delivered) and the FM-S09 spike is resolved (#107), so the M7 image-recipe/build epic is unblocked on the spike's pins (`packer >= 1.15 < 2`, `proxmox >= 1.2.4 < 2`, operator-installed CLI, machine-readable output, and the `/api2/json` URL contract). The Lab leases/instances/cleanup epic depends on Lab templates and readiness (epic #13), which is now unblocked too. Next: the Lab templates epic (#13), beginning with the template resource and readiness model, or the leases epic (#14), per the maintainer's priority. Create the issue, post the approach, branch `fm-702-*`/`fm-710-*`. The integration VM and the PVE host are the same environment FM-600..701 used.
