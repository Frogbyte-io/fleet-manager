...
Fleet Lab owns infrastructure selection, provisioning, readiness, project/tool setup, resource leasing, command dispatch, artifact collection metadata, expiry, and cleanup. Proxmox runs VMs; external browser/desktop/hardware test frameworks perform tests.

## Main resources

| Resource | Purpose |
|---|---|
| Lab template | Versioned desired definition of provider template, constraints, bootstrap profile, readiness policy, default TTL, and cleanup strategy |
...
Templates reference provider resources by stable provider IDs and an observed fingerprint/version. “Latest template named win11” is not reproducible enough for an active lease.
...
The scheduler filters then ranks:
...
Initial fairness is bounded FIFO per priority with per-identity/project concurrency limits. Preemption is out of scope. Queued requests explain the blocking resource without exposing unrelated tenant details.

## Provisioning saga

1. Create lease and operation after authorization/audit intent.
2. Select placement and reserve capacity/resources/VMID.
3. Clone immutable template to a Fleet-namespaced guest.
...
9. Optionally clone and prepare the project through the M3/M4 operation plan.
...
Fleet can collect declared paths, command logs, test reports, screenshots, and result bundles. The database stores metadata and digest; bytes live in a configurable controller volume first and object storage later. Collection failure does not automatically suppress cleanup unless policy explicitly grants a bounded diagnostic hold.

## USB and physical resources
