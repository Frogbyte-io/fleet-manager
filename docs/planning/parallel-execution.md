...
**Checkpoint 2026-09-05 (session handoff).** Waves 1–5 are closed and pushed; every closed issue has a close-out comment on GitHub with acceptance-criterion evidence, and `cargo xtask verify` plus the policy scripts are green at the checkpoint commit. **FM-210 (#64) is implemented on the `fm-210-add-machine-onboarding` branch (PR #69): the staged draft/test/discover/review/add workflow with the durable `onboarding_drafts` store, the `machine.onboard.test`/`machine.onboard.discover` operation kinds, the explicit fingerprint confirmation with changed-key blocking, duplicate warnings, the defined draft cleanup (no secret values; cancel unpins only unregistered hosts), the `/machines/onboarding/drafts` API + regenerated client, `fleetctl machines onboard …`, and the web `OnboardingPanel`.** After FM-210 merges, the next agent starts **FM-211 (#65) — the fleetd service install** (needs a real Linux VM; its dependencies FM-202/FM-204/FM-205/FM-207 are merged). Operating notes for the handoff:

- **What FM-210 builds on:** the machine surface FM-209 delivered (see the FM-209 guarantee below) is the read surface the Add Machine flow's review step renders; `fleetctl machines list/get` and the web Machines panel already show redacted endpoints and derived status.
- **What FM-212 can rely on:** `Machines::register` over the onboarding use case assigns the Fleet ID and the SSH endpoint, and the endpoint's `verified_fingerprint` is confirmed at add time — the upgrade flow's identity-association step can check `machineStatus` flipping from `agentless` after install, and the draft store (`OnboardingPort`) is the place an enrollment handoff could stage from if needed.
- **Composition facts:** `compose_onboarding(pool, <data dir>/ssh)` is the onboarding wiring point; the trust adapter and the onboarding executor must share the controller's SSH work directory (one trust store). The OpenAPI document and the TS client are generated artifacts — never hand-edit; regenerate with the `xtask` step commands whenever the API changes. `build_router`/`serve`/`serve_on` grew an `onboarding` parameter (existing callers pass `None`).
- **Remaining after FM-210:** wave 6 continues (`FM-211` service install — needs a real Linux VM — then `FM-212` upgrade), then wave 7 (`FM-213`, optional). FM-214 stays deferred past the first Lab release.
...
| 6 | FM-210 → FM-211 → FM-212 | **FM-210 done** (2026-09-05, PR #69); FM-211 next |
