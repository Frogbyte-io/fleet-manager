...
**Checkpoint 2026-09-05 (session handoff).** Waves 1–5 are closed and pushed; every closed issue has a close-out comment on GitHub with acceptance-criterion evidence, and `cargo xtask verify` plus the policy scripts are green at the checkpoint commit. The next agent starts **FM-210 (#64) — the SSH Add Machine onboarding workflow**: its dependencies (FM-201, FM-203, FM-209) are all merged, so it is unblocked with no in-flight work to coordinate with. Operating notes for the handoff:
...
- **Composition facts:** `compose_node_services` (controller lib) is the one wiring point for node trust + gateway; the OpenAPI document and the TS client are generated artifacts — never hand-edit; regenerate with the `xtask` step commands whenever the API changes.
...
| 6 | FM-210 → FM-211 → FM-212 | FM-210 next |
