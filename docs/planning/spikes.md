...
| FM-S02 | Is Cedar the right embedded authorization engine for Fleet's permission, resource, tag, and project policies? | M1 | — (expected to produce ADR-0009) | FM-105, FM-106 | Native Rust permission catalog with deny-by-default checks and no external policy language |
...
| FM-S04 | How does `fleetd` install as a Windows service, expose a named pipe, and authenticate a local peer at least as strictly as Unix socket peer credentials? | M2 | ADR-0003 | FM-208, FM-214 | Document Windows as inventory-only via SSH until the broker can be secured |
